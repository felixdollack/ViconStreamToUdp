...
//_readThread = gcnew Thread(gcnew ThreadStart(MarkerReader(MyClient)));
void MarkerReader(void *dummy);

//#define TARGETIP "10.0.0.2"
#define TARGETIP "192.168.12.14"
#define TARGETPORT 5555
...
int send_HALsocket(float x, float y, float z, float angle) {
	...
	//std::cout << "sent " << sock_counter++ << std::endl;
	return 0;
}
...
int main(int argc, char* argv[])
{
	...
	for(int i=0; i != 3; ++i) // repeat to check disconnecting doesn't wreck next connect
	{
  ...
  #endif
		{
			...
			send_HALsocket(translation.Translation[0], translation.Translation[1], translation.Translation[2], angles.Rotation[1]/PI*180);
